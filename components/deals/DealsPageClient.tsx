...
export function DealsPageClient({ initialCollection }: DealsPageClientProps) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    // Mapping categories to product types (simplified)
    ...
    return sortedProducts.filter((p) => {
        // Match against productType or tags as fallback
  ...
  const handleLoadMore = async () => {
    ...
    try {
      const nextCollection = await getCollectionPage("deals", pageInfo.endCursor || undefined);
      if (nextCollection) {
        const newProducts = nextCollection.products.edges.map((e) => e.node);
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      {/* Featured Section — only show top 3 when on "All Deals" */}
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between mb-8 md:mb-12">
            <h2 className="text-xl md:text-3xl font-black text-[#0a0f1e] uppercase tracking-tight">
              {activeCategory} <span className="text-[#ff4d00]/40 text-sm ml-2 font-bold tracking-widest">{filteredProducts.length} ITEMS</span>
            </h2>
          </div>
