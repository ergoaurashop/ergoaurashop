-- ============================================================================
-- E R G O A U R A   S H O P   —   C O M P L E T E   S U P A B A S E   S C H E M A
-- ============================================================================
-- Run this entire file in Supabase SQL Editor, in order.
-- This covers: tables, functions, triggers, indexes, RLS, seed data.
-- ============================================================================
...
-- 2a. profiles — linked to Supabase Auth (auth.users)
...
-- 3a. generate_track_id — 12-char alphanumeric
...
-- 3b. generate_order_id — readable ORD-XXXXXXXX
...
-- 3c. is_admin — helper for RLS policies
...
-- 3d. update_updated_at_column — generic trigger function
...
-- 4b. Log status changes to order_status_history
...
-- Allow trigger/service-role to insert profiles (on signup)
...
-- Public lookup by track_id is allowed (rate-limited in app)
...
-- ############################################################################
-- STEP 8: Seed product data (all 11 products from codebase)
-- ############################################################################
-- IMPORTANT: These use deterministic UUIDs so references stay stable.
-- If products already exist, this INSERT will be skipped (ON CONFLICT).
...
-- ############################################################################
-- STEP 9: Create an admin user reference
-- ############################################################################
-- NOTE: You cannot INSERT into auth.users directly via SQL.
-- You must first sign up via the Supabase Auth UI or API, then run:
--
--   UPDATE profiles SET is_admin = true WHERE email = 'your-admin@email.com';
--
-- This will be done after you create your admin account via the signup page.


-- ############################################################################
-- STEP 10: Insert a few sample orders for testing the admin dashboard
-- ############################################################################
-- These use the auto-generated IDs from products seeded above.
...
-- Sample order 4: B2G1 discount applied
INSERT INTO orders (
  customer_name, customer_email, customer_phone,
  address, products, subtotal, discount, shipping, total,
  payment_id, payment_status, order_status,
...
--   2. Run: UPDATE profiles SET is_admin = true WHERE email = 'your-email';
--   3. Your Supabase is production-ready!
-- ============================================================================
