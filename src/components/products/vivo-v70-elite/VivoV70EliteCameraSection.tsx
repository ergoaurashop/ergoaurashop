...
export default function VivoV70EliteCameraSection() {
  const { highlights, features } = VIVO_V70_ELITE_CAMERA_CONTENT;
  ...
  const breakImage = `/images/products/${VIVO_V70_ELITE_FOLDER}/${VIVO_V70_ELITE_PRODUCT_IMAGES[6]}`;

  return (
    <section className="vivov70elite-section" id="vivov70elite-camera">
      <div className="vivov70elite-section-container">
        ...
        >
          <span className="vivov70elite-section-label">Camera System</span>
          <h2 className="vivov70elite-section-title">
            {VIVO_V70_ELITE_CAMERA_CONTENT.title}
          </h2>
          <p className="vivov70elite-section-subtitle mx-auto">
            {VIVO_V70_ELITE_CAMERA_CONTENT.subtitle}
          </p>
        </motion.div>
            ...
            <div key={lens.label}>
              ...
              >
                ...
                >
                  ...
                  <h3 className="vivov70elite-alternating-title">{lens.label}</h3>
                  <p className="vivov70elite-alternating-desc">{lens.detail}</p>
                </motion.div>
                <motion.div
                  initial={{ opacity: 0, x: i % 2 === 0 ? 40 : -40 }}
                  whileInView={{ opacity: 1, x: 0 }}
              ...
              {i === 1 && breakImage && (
                <div className="vivov70elite-fullwidth-break my-12">
                  <Image
                    src={breakImage}
                    alt="vivo V70 Elite camera system"
                    fill
                    sizes="100vw"
                    style={{ objectFit: "cover" }}
                  />
        ...
        <motion.div
          ...
          className="mt-16"
        >
          <div className="vivov70elite-glass-card max-w-3xl mx-auto">
            ...
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {features.map((feature, i) => (
                <li
                  key={i}
                  className="flex items-start gap-2 text-sm text-[var(--vivov70elite-text-tertiary)]"
                >
                  <svg
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    className="w-4 h-4 mt-0.5 shrink-0 text-[var(--vivov70elite-accent-text)]"
                  >
