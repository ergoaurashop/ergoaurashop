...
function StarRow({ rating }: { rating: number }) {
  return (
    <div className="p4power-review-stars">
      {[1, 2, 3, 4, 5].map((star) => (
        <span key={star} className="p4power-review-star">
...
function ReviewCard({ review }: { review: ProductReviewDetail }) {
  ...
  const reviewImages = P4POWER_REVIEW_IMAGES[review.id] || [];
  ...
  return (
    ...
    >
      <div className="p4power-review-header">
        {/* Blue avatar with initials */}
        <div className="p4power-review-avatar">{getInitials(review.name)}</div>
        <div>
          <div className="p4power-review-name">{review.name}</div>
          <div className="p4power-review-city">{review.city}</div>
        </div>
      </div>
      ...
      <h4 className="p4power-review-title">{review.title}</h4>
      <p className="p4power-review-text">{review.text}</p>

      {/* ── Review photo carousel ── */}
      {reviewImages.length > 0 && (
        <div className="p4power-review-images">
          {reviewImages.map((filename, i) => (
            <div
              key={filename}
              className="p4power-review-image-wrapper"
      ...
      {reviewImages.length > 1 && (
        <div className="p4power-review-carousel-nav">
          <button
            className="p4power-review-carousel-btn"
            onClick={() =>
              setCurrentImg((prev) =>
          ...
          <div className="p4power-review-carousel-dots">
            {reviewImages.map((_, i) => (
              <button
                key={i}
                className={`p4power-review-carousel-dot${i === currentImg ? " active" : ""}`}
                onClick={() => {
                  setCurrentImg(i);
                  openLightbox(i);
                }}
      ...
      <div className="p4power-review-meta">
        {review.isVerified && (
          <span className="p4power-review-verified">
            <VerifiedCheckmark />
            Verified Purchase
          </span>
        )}
        <button
          ...
          className={`p4power-review-helpful ${thumbsUp ? "thumbs-up-active" : ""}`}
      ...
      {lightboxOpen && (
        <div
          className="p4power-review-lightbox"
        ...
        >
          <button
            className="p4power-review-lightbox-close"
            onClick={closeLightbox}
...
function RatingBarRow({
  ...
  return (
    <div
      className={`p4power-rating-row ${isActive ? "active" : ""}`}
      onClick={onClick}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => {
    ...
    >
      <span className="p4power-rating-label">{star}★</span>
      <div className="p4power-rating-bar-track">
        <div className="p4power-rating-bar-fill" style={{ width: `${pct}%` }} />
      </div>
      <span className="p4power-rating-count">{count}</span>
    </div>
  );
}

/* ── Main Reviews Component ── */
export default function P4PowerReviews() {
  ...
  const summary = P4POWER_REVIEW_SUMMARY;

  // Filter
  let filtered = filterStar
    ? P4POWER_REVIEWS.filter((r) => r.rating === filterStar)
    : [...P4POWER_REVIEWS];
  ...
  return (
    <section className="p4power-reviews-section" id="p4power-reviews">
      <div className="p4power-reviews-container">
        <motion.div
        ...
        >
          ...
          <div className="text-center mb-8">
            <h2 className="p4power-section-title">Customer Reviews</h2>
          ...
          <div className="p4power-rating-bars">
            <div className="p4power-rating-summary">
              <div className="p4power-average-rating">
                {summary.averageRating.toFixed(1)}
              </div>
              <div>
                <div className="p4power-average-stars">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <span key={star} className="p4power-review-star">
                      <StarIcon
                ...
                <div className="p4power-total-reviews">
            ...
            <div className="p4power-rating-bars-title">
          ...
          {/* Toolbar: count + sort */}
          <div className="p4power-reviews-toolbar">
            <span className="p4power-reviews-count">
              {filtered.length} review{filtered.length !== 1 ? "s" : ""}
              {filterStar ? ` (${filterStar}★)` : ""}
            </span>
            <div className="p4power-reviews-sort">
              <label htmlFor="p4power-sort">Sort by:</label>
              <select
                id="p4power-sort"
                value={sortBy}
                onChange={(e) =>
          ...
          {filtered.length > 8 && (
            <button
              ...
              className="p4power-reviews-load-more"
            >
