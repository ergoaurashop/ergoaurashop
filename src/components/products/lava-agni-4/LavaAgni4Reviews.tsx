...
function StarRow({ rating }: { rating: number }) {
  return (
    <div className="lavaagni4-review-stars">
      {[1, 2, 3, 4, 5].map((star) => (
        <span key={star} className="lavaagni4-review-star">
...
function ReviewCard({ review }: { review: ProductReviewDetail }) {
  ...
  const reviewImages = LAVA_AGNI_4_REVIEW_IMAGES[review.id] || [];
  ...
  return (
    <motion.div
      className="lavaagni4-review-card"
      layout
    ...
    >
      <div className="lavaagni4-review-header">
        {/* Blue avatar with initials */}
        <div className="lavaagni4-review-avatar">{getInitials(review.name)}</div>
        <div>
          <div className="lavaagni4-review-name">{review.name}</div>
          <div className="lavaagni4-review-city">{review.city}</div>
        </div>
      </div>
      ...
      <h4 className="lavaagni4-review-title">{review.title}</h4>
      <p className="lavaagni4-review-text">{review.text}</p>

      {/* ── Review photo carousel ── */}
      {reviewImages.length > 0 && (
        <div className="lavaagni4-review-images">
          {reviewImages.map((filename, i) => (
            <div
              key={filename}
              className="lavaagni4-review-image-wrapper"
      ...
      {reviewImages.length > 1 && (
        <div className="lavaagni4-review-carousel-nav">
          <button
            className="lavaagni4-review-carousel-btn"
            onClick={() =>
              setCurrentImg((prev) =>
          ...
          <div className="lavaagni4-review-carousel-dots">
            {reviewImages.map((_, i) => (
              <button
                key={i}
                className={`lavaagni4-review-carousel-dot${i === currentImg ? " active" : ""}`}
                onClick={() => {
                  setCurrentImg(i);
                  openLightbox(i);
                }}
      ...
      <div className="lavaagni4-review-meta">
        {review.isVerified && (
          <span className="lavaagni4-review-verified">
            <VerifiedCheckmark />
            Verified Purchase
          </span>
        )}
        <button
          ...
          className={`lavaagni4-review-helpful ${thumbsUp ? "thumbs-up-active" : ""}`}
      ...
      {lightboxOpen && (
        <div
          className="lavaagni4-review-lightbox"
        ...
        >
          <button
            className="lavaagni4-review-lightbox-close"
            onClick={closeLightbox}
...
function RatingBarRow({
  ...
  return (
    <div
      className={`lavaagni4-rating-row ${isActive ? "active" : ""}`}
      onClick={onClick}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => {
    ...
    >
      <span className="lavaagni4-rating-label">{star}★</span>
      <div className="lavaagni4-rating-bar-track">
        <div className="lavaagni4-rating-bar-fill" style={{ width: `${pct}%` }} />
      </div>
      <span className="lavaagni4-rating-count">{count}</span>
    </div>
  );
}

/* ── Main Reviews Component ── */
export default function LavaAgni4Reviews() {
  ...
  const summary = LAVA_AGNI_4_REVIEW_SUMMARY;

  // Filter
  let filtered = filterStar
    ? LAVA_AGNI_4_REVIEWS.filter((r) => r.rating === filterStar)
    : [...LAVA_AGNI_4_REVIEWS];
  ...
  return (
    <section className="lavaagni4-reviews-section" id="lavaagni4-reviews">
      <div className="lavaagni4-reviews-container">
        <motion.div
        ...
        >
          ...
          <div className="text-center mb-8">
            <h2 className="lavaagni4-section-title">Customer Reviews</h2>
          ...
          <div className="lavaagni4-rating-bars">
            <div className="lavaagni4-rating-summary">
              <div className="lavaagni4-average-rating">
                {summary.averageRating.toFixed(1)}
              </div>
              <div>
                <div className="lavaagni4-average-stars">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <span key={star} className="lavaagni4-review-star">
                      <StarIcon
                ...
                <div className="lavaagni4-total-reviews">
            ...
            <div className="lavaagni4-rating-bars-title">
          ...
          {/* Toolbar: count + sort */}
          <div className="lavaagni4-reviews-toolbar">
            <span className="lavaagni4-reviews-count">
              {filtered.length} review{filtered.length !== 1 ? "s" : ""}
              {filterStar ? ` (${filterStar}★)` : ""}
            </span>
            <div className="lavaagni4-reviews-sort">
              <label htmlFor="lavaagni4-sort">Sort by:</label>
              <select
                id="lavaagni4-sort"
                value={sortBy}
                onChange={(e) =>
          ...
          {filtered.length > 8 && (
            <button
              ...
              className="lavaagni4-reviews-load-more"
            >
