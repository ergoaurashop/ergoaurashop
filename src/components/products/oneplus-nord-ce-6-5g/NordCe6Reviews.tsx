...
function StarRow({ rating }: { rating: number }) {
  return (
    <div className="nordce6-review-stars">
      {[1, 2, 3, 4, 5].map((star) => (
        <span key={star} className="nordce6-review-star">
...
function ReviewCard({ review }: { review: ProductReviewDetail }) {
  ...
  const reviewImages = NORD_CE6_REVIEW_IMAGES[review.id] || [];
  ...
  return (
    ...
    >
      <div className="nordce6-review-header">
        {/* Blue avatar with initials */}
        <div className="nordce6-review-avatar">{getInitials(review.name)}</div>
        <div>
          <div className="nordce6-review-name">{review.name}</div>
          <div className="nordce6-review-city">{review.city}</div>
        </div>
      </div>
      ...
      <h4 className="nordce6-review-title">{review.title}</h4>
      <p className="nordce6-review-text">{review.text}</p>

      {/* ── Review photo carousel ── */}
      {reviewImages.length > 0 && (
        <div className="nordce6-review-images">
          {reviewImages.map((filename, i) => (
            <div
              key={filename}
              className="nordce6-review-image-wrapper"
      ...
      {reviewImages.length > 1 && (
        <div className="nordce6-review-carousel-nav">
          <button
            className="nordce6-review-carousel-btn"
            onClick={() =>
              setCurrentImg((prev) =>
          ...
          <div className="nordce6-review-carousel-dots">
            {reviewImages.map((_, i) => (
              <button
                key={i}
                className={`nordce6-review-carousel-dot${i === currentImg ? " active" : ""}`}
                onClick={() => {
                  setCurrentImg(i);
                  openLightbox(i);
                }}
      ...
      <div className="nordce6-review-meta">
        {review.isVerified && (
          <span className="nordce6-review-verified">
            <VerifiedCheckmark />
            Verified Purchase
          </span>
        )}
        <button
          ...
          className={`nordce6-review-helpful ${thumbsUp ? "thumbs-up-active" : ""}`}
      ...
      {lightboxOpen && (
        <div
          className="nordce6-review-lightbox"
        ...
        >
          <button
            className="nordce6-review-lightbox-close"
            onClick={closeLightbox}
...
function RatingBarRow({
  ...
  return (
    <div
      className={`nordce6-rating-row ${isActive ? "active" : ""}`}
      onClick={onClick}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => {
    ...
    >
      <span className="nordce6-rating-label">{star}★</span>
      <div className="nordce6-rating-bar-track">
        <div className="nordce6-rating-bar-fill" style={{ width: `${pct}%` }} />
      </div>
      <span className="nordce6-rating-count">{count}</span>
    </div>
  );
}

/* ── Main Reviews Component ── */
export default function NordCe6Reviews() {
  ...
  const summary = NORD_CE6_REVIEW_SUMMARY;

  // Filter
  let filtered = filterStar
    ? NORD_CE6_REVIEWS.filter((r) => r.rating === filterStar)
    : [...NORD_CE6_REVIEWS];
  ...
  return (
    <section className="nordce6-reviews-section" id="nordce6-reviews">
      <div className="nordce6-reviews-container">
        <motion.div
        ...
        >
          ...
          <div className="text-center mb-8">
            <h2 className="nordce6-section-title">Customer Reviews</h2>
          ...
          <div className="nordce6-rating-bars">
            <div className="nordce6-rating-summary">
              <div className="nordce6-average-rating">
                {summary.averageRating.toFixed(1)}
              </div>
              <div>
                <div className="nordce6-average-stars">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <span key={star} className="nordce6-review-star">
                      <StarIcon
                ...
                <div className="nordce6-total-reviews">
            ...
            <div className="nordce6-rating-bars-title">
          ...
          {/* Toolbar: count + sort */}
          <div className="nordce6-reviews-toolbar">
            <span className="nordce6-reviews-count">
              {filtered.length} review{filtered.length !== 1 ? "s" : ""}
              {filterStar ? ` (${filterStar}★)` : ""}
            </span>
            <div className="nordce6-reviews-sort">
              <label htmlFor="nordce6-sort">Sort by:</label>
              <select
                id="nordce6-sort"
                value={sortBy}
                onChange={(e) =>
          ...
          {filtered.length > 8 && (
            <button
              ...
              className="nordce6-reviews-load-more"
            >
