...
export default function Edge70CameraSection() {
  const { highlights, features } = EDGE_70_CAMERA_CONTENT;
  ...
  const breakImage = `/images/products/${EDGE_70_FOLDER}/${EDGE_70_PRODUCT_IMAGES[6]}`;

  return (
    <section className="edge70-section" id="edge70-camera">
      <div className="edge70-section-container">
        ...
        >
          <span className="edge70-section-label">Camera System</span>
          <h2 className="edge70-section-title">
            {EDGE_70_CAMERA_CONTENT.title}
          </h2>
          <p className="edge70-section-subtitle mx-auto">
            {EDGE_70_CAMERA_CONTENT.subtitle}
          </p>
        </motion.div>
            ...
            <div key={lens.label}>
              ...
              >
                ...
                >
                  ...
                  <h3 className="edge70-alternating-title">{lens.label}</h3>
                  <p className="edge70-alternating-desc">{lens.detail}</p>
                </motion.div>
                <motion.div
                  initial={{ opacity: 0, x: i % 2 === 0 ? 40 : -40 }}
                  whileInView={{ opacity: 1, x: 0 }}
              ...
              {i === 1 && breakImage && (
                <div className="edge70-fullwidth-break my-12">
                  <Image
                    src={breakImage}
                    alt="Motorola Edge 70 camera system"
                    fill
                    sizes="100vw"
                    style={{ objectFit: "cover" }}
                  />
        ...
        <motion.div
          ...
          className="mt-16"
        >
          <div className="edge70-glass-card max-w-3xl mx-auto">
            ...
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {features.map((feature, i) => (
                <li
                  key={i}
                  className="flex items-start gap-2 text-sm text-[var(--edge70-text-tertiary)]"
                >
                  <svg
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    className="w-4 h-4 mt-0.5 shrink-0 text-[var(--edge70-accent-text)]"
                  >
