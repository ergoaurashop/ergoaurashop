...
function StarRow({ rating }: { rating: number }) {
  return (
    <div className="edge70-review-stars">
      {[1, 2, 3, 4, 5].map((star) => (
        <span key={star} className="edge70-review-star">
...
function ReviewCard({ review }: { review: ProductReviewDetail }) {
  ...
  const reviewImages = EDGE_70_REVIEW_IMAGES[review.id] || [];
  ...
  return (
    ...
    >
      <div className="edge70-review-header">
        {/* Blue avatar with initials */}
        <div className="edge70-review-avatar">{getInitials(review.name)}</div>
        <div>
          <div className="edge70-review-name">{review.name}</div>
          <div className="edge70-review-city">{review.city}</div>
        </div>
      </div>
      ...
      <h4 className="edge70-review-title">{review.title}</h4>
      <p className="edge70-review-text">{review.text}</p>

      {/* ── Review photo carousel ── */}
      {reviewImages.length > 0 && (
        <div className="edge70-review-images">
          {reviewImages.map((filename, i) => (
            <div
              key={filename}
              className="edge70-review-image-wrapper"
      ...
      {reviewImages.length > 1 && (
        <div className="edge70-review-carousel-nav">
          <button
            className="edge70-review-carousel-btn"
            onClick={() =>
              setCurrentImg((prev) =>
          ...
          <div className="edge70-review-carousel-dots">
            {reviewImages.map((_, i) => (
              <button
                key={i}
                className={`edge70-review-carousel-dot${i === currentImg ? " active" : ""}`}
                onClick={() => {
                  setCurrentImg(i);
                  openLightbox(i);
                }}
      ...
      <div className="edge70-review-meta">
        {review.isVerified && (
          <span className="edge70-review-verified">
            <VerifiedCheckmark />
            Verified Purchase
          </span>
        )}
        <button
          ...
          className={`edge70-review-helpful ${thumbsUp ? "thumbs-up-active" : ""}`}
      ...
      {lightboxOpen && (
        <div
          className="edge70-review-lightbox"
        ...
        >
          <button
            className="edge70-review-lightbox-close"
            onClick={closeLightbox}
...
function RatingBarRow({
  ...
  return (
    <div
      className={`edge70-rating-row ${isActive ? "active" : ""}`}
      onClick={onClick}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => {
    ...
    >
      <span className="edge70-rating-label">{star}★</span>
      <div className="edge70-rating-bar-track">
        <div className="edge70-rating-bar-fill" style={{ width: `${pct}%` }} />
      </div>
      <span className="edge70-rating-count">{count}</span>
    </div>
  );
}

/* ── Main Reviews Component ── */
export default function Edge70Reviews() {
  ...
  const summary = EDGE_70_REVIEW_SUMMARY;

  // Filter
  let filtered = filterStar
    ? EDGE_70_REVIEWS.filter((r) => r.rating === filterStar)
    : [...EDGE_70_REVIEWS];
  ...
  return (
    <section className="edge70-reviews-section" id="edge70-reviews">
      <div className="edge70-reviews-container">
        <motion.div
        ...
        >
          ...
          <div className="text-center mb-8">
            <h2 className="edge70-section-title">Customer Reviews</h2>
          ...
          <div className="edge70-rating-bars">
            <div className="edge70-rating-summary">
              <div className="edge70-average-rating">
                {summary.averageRating.toFixed(1)}
              </div>
              <div>
                <div className="edge70-average-stars">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <span key={star} className="edge70-review-star">
                      <StarIcon
                ...
                <div className="edge70-total-reviews">
            ...
            <div className="edge70-rating-bars-title">
          ...
          {/* Toolbar: count + sort */}
          <div className="edge70-reviews-toolbar">
            <span className="edge70-reviews-count">
              {filtered.length} review{filtered.length !== 1 ? "s" : ""}
              {filterStar ? ` (${filterStar}★)` : ""}
            </span>
            <div className="edge70-reviews-sort">
              <label htmlFor="edge70-sort">Sort by:</label>
              <select
                id="edge70-sort"
                value={sortBy}
                onChange={(e) =>
          ...
          {filtered.length > 8 && (
            <button
              ...
              className="edge70-reviews-load-more"
            >
