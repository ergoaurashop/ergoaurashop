...
function StarRow({ rating }: { rating: number }) {
  return (
    <div className="iphone-review-stars">
      {[1, 2, 3, 4, 5].map((star) => (
        <span key={star} className="iphone-review-star">
...
function ReviewCard({ review }: { review: ProductReviewDetail }) {
  ...
  const reviewImages = IPHONE_REVIEW_IMAGES[review.id] || [];
  ...
  return (
    ...
    >
      <div className="iphone-review-header">
        {/* Blue avatar with initials */}
        <div className="iphone-review-avatar">{getInitials(review.name)}</div>
        <div>
          <div className="iphone-review-name">{review.name}</div>
          <div className="iphone-review-city">{review.city}</div>
        </div>
      </div>
      ...
      <h4 className="iphone-review-title">{review.title}</h4>
      <p className="iphone-review-text">{review.text}</p>

      {/* ── Review photo carousel ── */}
      {reviewImages.length > 0 && (
        <div className="iphone-review-images">
          {reviewImages.map((filename, i) => (
            <div
              key={filename}
              className="iphone-review-image-wrapper"
      ...
      {reviewImages.length > 1 && (
        <div className="iphone-review-carousel-nav">
          <button
            className="iphone-review-carousel-btn"
            onClick={() =>
              setCurrentImg((prev) =>
          ...
          <div className="iphone-review-carousel-dots">
            {reviewImages.map((_, i) => (
              <button
                key={i}
                className={`iphone-review-carousel-dot${i === currentImg ? " active" : ""}`}
                onClick={() => {
                  setCurrentImg(i);
                  openLightbox(i);
                }}
      ...
      <div className="iphone-review-meta">
        {review.isVerified && (
          <span className="iphone-review-verified">
            <VerifiedCheckmark />
            Verified Purchase
          </span>
        )}
        <button
          ...
          className={`iphone-review-helpful ${thumbsUp ? "thumbs-up-active" : ""}`}
      ...
      {lightboxOpen && (
        <div
          className="iphone-review-lightbox"
        ...
        >
          <button
            className="iphone-review-lightbox-close"
            onClick={closeLightbox}
...
function RatingBarRow({
  ...
  return (
    <div
      className={`iphone-rating-row ${isActive ? "active" : ""}`}
      onClick={onClick}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => {
    ...
    >
      <span className="iphone-rating-label">{star}★</span>
      <div className="iphone-rating-bar-track">
        <div className="iphone-rating-bar-fill" style={{ width: `${pct}%` }} />
      </div>
      <span className="iphone-rating-count">{count}</span>
    </div>
  );
}

/* ── Main Reviews Component ── */
export default function IPhoneReviews() {
  ...
  const summary = IPHONE_REVIEW_SUMMARY;

  // Filter
  let filtered = filterStar
    ? IPHONE_REVIEWS.filter((r) => r.rating === filterStar)
    : [...IPHONE_REVIEWS];
  ...
  return (
    <section className="iphone-reviews-section" id="iphone-reviews">
      <div className="iphone-reviews-container">
        <motion.div
        ...
        >
          ...
          <div className="iphone-rating-bars">
            <div className="iphone-rating-summary">
              <div className="iphone-average-rating">
                {summary.averageRating.toFixed(1)}
              </div>
              <div>
                <div className="iphone-average-stars">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <span key={star} className="iphone-review-star">
                      <StarIcon
                ...
                <div className="iphone-total-reviews">
            ...
            <div className="iphone-rating-bars-title">
          ...
          {/* Toolbar: count + sort */}
          <div className="iphone-reviews-toolbar">
            <span className="iphone-reviews-count">
              {filtered.length} review{filtered.length !== 1 ? "s" : ""}
              {filterStar ? ` (${filterStar}★)` : ""}
            </span>
            <div className="iphone-reviews-sort">
              <label htmlFor="iphone-sort">Sort by:</label>
              <select
                id="iphone-sort"
                value={sortBy}
                onChange={(e) =>
          ...
          {filtered.length > 8 && (
            <button
              ...
              className="iphone-reviews-load-more"
            >
