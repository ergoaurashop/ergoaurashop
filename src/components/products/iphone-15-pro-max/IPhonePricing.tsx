...
import {
  IPHONE_PRODUCT,
  IPHONE_COLOUR_OPTIONS,
} from "@/lib/iphone-15-pro-max-data";
import { formatPrice } from "@/lib/utils";
import { useCartStore } from "@/store/cartStore";

interface IPhonePricingProps {
  id?: string;
}
...
function TimerDigit({ value, label }: { value: number; label: string }) {
  ...
  return (
    <div className="iphone-timer-unit">
      <div
        className={`iphone-timer-digit${ticking ? " ticking" : ""}`}
        key={value}
      >
        {String(value).padStart(2, "0")}
      </div>
      <span className="iphone-timer-label">{label}</span>
    </div>
  );
}

export default function IPhonePricing({ id }: IPhonePricingProps) {
  ...
  const savings = IPHONE_PRODUCT.original_price - IPHONE_PRODUCT.price;

  // Split title into words for individual animation
  const titleWords = "Grab Yours Before It's Gone".split(" ");

  return (
    <section className="iphone-pricing-section" id={id}>
      {/* Sparkle particles */}
      <div className="iphone-pricing-sparkles">
        <div className="iphone-sparkle" />
        <div className="iphone-sparkle" />
        <div className="iphone-sparkle" />
        <div className="iphone-sparkle" />
        <div className="iphone-sparkle" />
      ...
      <div
        className="iphone-section-container"
      ...
      >
        ...
        >
          {/* Animated label */}
          <span className="iphone-pricing-label">Limited Time Offer</span>

          {/* Title with word-by-word highlight effects */}
          <h2 className="iphone-pricing-title">
          ...
          <p className="iphone-section-subtitle mx-auto mb-8">
            Only {IPHONE_PRODUCT.stock} units remaining at this price. Once
            they're sold, this deal is gone forever.
          </p>

          {/* Price display with glow and ring effects */}
          <div className="iphone-pricing-prices">
            <span className="iphone-price-original">
              {formatPrice(IPHONE_PRODUCT.original_price)}
            </span>

            <span className="iphone-price-current-glow">
              <span className="iphone-price-current">
                {formatPrice(IPHONE_PRODUCT.price)}
              </span>
              <div className="iphone-price-glow-ring" />
            </span>

            <span className="iphone-price-badge-ring">
          ...
          {/* Savings */}
          <p className="iphone-pricing-savings mb-8">
            <svg
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
          ...
          {/* Colour selector */}
          {IPHONE_COLOUR_OPTIONS.length > 0 && (
              ...
              <div className="iphone-colour-options">
                {IPHONE_COLOUR_OPTIONS.map((colour, i) => (
                  <button
                    ...
                    className={`iphone-colour-swatch ${
          ...
          <div className="iphone-pricing-timer">
            <div className="iphone-pricing-timer-header">
              <svg
                width="16"
                height="16"
                viewBox="0 0 24 24"
            ...
            <div className="iphone-timer-units">
              <TimerDigit value={timeLeft.hours} label="Hours" />
              <span className="iphone-timer-separator">:</span>
          ...
          {/* Stock indicator */}
          <div className="iphone-pricing-stock">
            <div className="stock-header">
              <span className="stock-count">
                Only{" "}
                <span className="count-number">{IPHONE_PRODUCT.stock}</span>{" "}
                left in stock
              </span>
            ...
            <div className="iphone-stock-bar">
              <div className="iphone-stock-fill" style={{ width: "15%" }} />
            </div>
          </div>

          {/* Trust badges */}
          <div className="iphone-pricing-trust">
            <motion.span
