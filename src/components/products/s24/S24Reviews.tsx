...
function StarRow({ rating }: { rating: number }) {
  return (
    <div className="s24-review-stars">
      {[1, 2, 3, 4, 5].map((star) => (
        <span key={star} className="s24-review-star">
...
function ReviewCard({ review }: { review: ProductReviewDetail }) {
  ...
  const reviewImages = S24_REVIEW_IMAGES[review.id] || [];
  ...
  return (
    ...
    >
      <div className="s24-review-header">
        {/* Green avatar with initials */}
        <div className="s24-review-avatar">{getInitials(review.name)}</div>
        <div>
          <div className="s24-review-name">{review.name}</div>
          <div className="s24-review-city">{review.city}</div>
        </div>
      </div>
      ...
      <h4 className="s24-review-title">{review.title}</h4>
      <p className="s24-review-text">{review.text}</p>

      {/* ── Review photo carousel (only for reviews with images) ── */}
      {reviewImages.length > 0 && (
        <div className="s24-review-images">
          {reviewImages.map((filename, i) => (
            <div
              key={filename}
              className="s24-review-image-wrapper"
      ...
      {reviewImages.length > 1 && (
        <div className="s24-review-carousel-nav">
          <button
            className="s24-review-carousel-btn"
            onClick={() =>
              setCurrentImg((prev) =>
          ...
          <div className="s24-review-carousel-dots">
            {reviewImages.map((_, i) => (
              <button
                key={i}
                className={`s24-review-carousel-dot${i === currentImg ? " active" : ""}`}
                onClick={() => {
                  setCurrentImg(i);
                  openLightbox(i);
                }}
      ...
      <div className="s24-review-meta">
        {review.isVerified && (
          <span className="s24-review-verified">
            <VerifiedCheckmark />
            Verified Purchase
          </span>
        )}
        ...
        <button
          ...
          className={`s24-review-helpful ${thumbsUp ? "thumbs-up-active" : ""}`}
      ...
      {lightboxOpen && (
        ...
        >
          <button
            className="s24-review-lightbox-close"
            onClick={closeLightbox}
...
function RatingBarRow({
  ...
  return (
    <div
      className={`s24-rating-row ${isActive ? "active" : ""}`}
      onClick={onClick}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => {
    ...
    >
      <span className="s24-rating-label">{star}★</span>
      <div className="s24-rating-bar-track">
        <div className="s24-rating-bar-fill" style={{ width: `${pct}%` }} />
      </div>
      <span className="s24-rating-count">{count}</span>
    </div>
  );
}

/* ── Main Reviews Component ── */
export default function S24Reviews() {
  ...
  const summary = S24_REVIEW_SUMMARY;

  // Filter
  let filtered = filterStar
    ? S24_REVIEWS.filter((r) => r.rating === filterStar)
    : [...S24_REVIEWS];
  ...
  return (
    <section className="s24-reviews-section" id="s24-reviews">
      <div className="s24-reviews-container">
        <motion.div
        ...
        >
          ...
          <div className="text-center mb-8">
            <h2 className="s24-section-title" style={{ color: "#ffffff" }}>
              Customer Reviews
            </h2>
          ...
          <div className="s24-rating-bars">
            <div className="s24-rating-summary">
              <div className="s24-average-rating">
                {summary.averageRating.toFixed(1)}
              </div>
              <div>
                <div className="s24-average-stars">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <span key={star} className="s24-review-star">
                      <StarIcon
                ...
                <div className="s24-total-reviews">
            ...
            <div className="s24-rating-bars-title">Filter by star rating</div>
          ...
          {/* Toolbar: count + sort */}
          <div className="s24-reviews-toolbar">
            <span className="s24-reviews-count">
              {filtered.length} review{filtered.length !== 1 ? "s" : ""}
              {filterStar ? ` (${filterStar}★)` : ""}
            </span>
            <div className="s24-reviews-sort">
              <label htmlFor="s24-sort">Sort by:</label>
              <select
                id="s24-sort"
                value={sortBy}
                onChange={(e) =>
          ...
          {filtered.length > 8 && (
            <button
              ...
              className="s24-reviews-load-more"
            >
