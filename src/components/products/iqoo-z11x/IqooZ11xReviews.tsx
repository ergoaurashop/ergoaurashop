...
function StarRow({ rating }: { rating: number }) {
  return (
    <div className="iqooz11x-review-stars">
      {[1, 2, 3, 4, 5].map((star) => (
        <span key={star} className="iqooz11x-review-star">
...
function ReviewCard({ review }: { review: ProductReviewDetail }) {
  ...
  const reviewImages = IQOO_Z11X_REVIEW_IMAGES[review.id] || [];
  ...
  return (
    <motion.div
      className="iqooz11x-review-card"
      layout
    ...
    >
      <div className="iqooz11x-review-header">
        {/* Blue avatar with initials */}
        <div className="iqooz11x-review-avatar">{getInitials(review.name)}</div>
        <div>
          <div className="iqooz11x-review-name">{review.name}</div>
          <div className="iqooz11x-review-city">{review.city}</div>
        </div>
      </div>
      ...
      <h4 className="iqooz11x-review-title">{review.title}</h4>
      <p className="iqooz11x-review-text">{review.text}</p>

      {/* ── Review photo carousel ── */}
      {reviewImages.length > 0 && (
        <div className="iqooz11x-review-images">
          {reviewImages.map((filename, i) => (
            <div
              key={filename}
              className="iqooz11x-review-image-wrapper"
      ...
      {reviewImages.length > 1 && (
        <div className="iqooz11x-review-carousel-nav">
          <button
            className="iqooz11x-review-carousel-btn"
            onClick={() =>
              setCurrentImg((prev) =>
          ...
          <div className="iqooz11x-review-carousel-dots">
            {reviewImages.map((_, i) => (
              <button
                key={i}
                className={`iqooz11x-review-carousel-dot${i === currentImg ? " active" : ""}`}
                onClick={() => {
                  setCurrentImg(i);
                  openLightbox(i);
                }}
      ...
      <div className="iqooz11x-review-meta">
        {review.isVerified && (
          <span className="iqooz11x-review-verified">
            <VerifiedCheckmark />
            Verified Purchase
          </span>
        )}
        <button
          ...
          className={`iqooz11x-review-helpful ${thumbsUp ? "thumbs-up-active" : ""}`}
      ...
      {lightboxOpen && (
        <div
          className="iqooz11x-review-lightbox"
        ...
        >
          <button
            className="iqooz11x-review-lightbox-close"
            onClick={closeLightbox}
...
function RatingBarRow({
  ...
  return (
    <div
      className={`iqooz11x-rating-row ${isActive ? "active" : ""}`}
      onClick={onClick}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => {
    ...
    >
      <span className="iqooz11x-rating-label">{star}★</span>
      <div className="iqooz11x-rating-bar-track">
        <div className="iqooz11x-rating-bar-fill" style={{ width: `${pct}%` }} />
      </div>
      <span className="iqooz11x-rating-count">{count}</span>
    </div>
  );
}

/* ── Main Reviews Component ── */
export default function IqooZ11xReviews() {
  ...
  const summary = IQOO_Z11X_REVIEW_SUMMARY;

  // Filter
  let filtered = filterStar
    ? IQOO_Z11X_REVIEWS.filter((r) => r.rating === filterStar)
    : [...IQOO_Z11X_REVIEWS];
  ...
  return (
    <section className="iqooz11x-reviews-section" id="iqooz11x-reviews">
      <div className="iqooz11x-reviews-container">
        <motion.div
        ...
        >
          ...
          <div className="text-center mb-8">
            <h2 className="iqooz11x-section-title">Customer Reviews</h2>
          ...
          <div className="iqooz11x-rating-bars">
            <div className="iqooz11x-rating-summary">
              <div className="iqooz11x-average-rating">
                {summary.averageRating.toFixed(1)}
              </div>
              <div>
                <div className="iqooz11x-average-stars">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <span key={star} className="iqooz11x-review-star">
                      <StarIcon
                ...
                <div className="iqooz11x-total-reviews">
            ...
            <div className="iqooz11x-rating-bars-title">
          ...
          {/* Toolbar: count + sort */}
          <div className="iqooz11x-reviews-toolbar">
            <span className="iqooz11x-reviews-count">
              {filtered.length} review{filtered.length !== 1 ? "s" : ""}
              {filterStar ? ` (${filterStar}★)` : ""}
            </span>
            <div className="iqooz11x-reviews-sort">
              <label htmlFor="iqooz11x-sort">Sort by:</label>
              <select
                id="iqooz11x-sort"
                value={sortBy}
                onChange={(e) =>
          ...
          {filtered.length > 8 && (
            <button
              ...
              className="iqooz11x-reviews-load-more"
            >
